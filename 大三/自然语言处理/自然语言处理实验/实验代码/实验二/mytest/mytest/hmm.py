...
file = open('./mytest/static/1998-01-2003版-带音.txt', encoding='utf-8').read()
file = '\n'.join(re.split('\n', file)[1:])
text = re.split('\n|。|！|？', file)
# print(text)
newText = []
...
trans_save = open('./mytest/static/trans.json')
trans = json.load(trans_save)
mixed_save = open('./mytest/static/mixed.json')
...
trans_save = open('./mytest/static/char_trans.json')
char_trans = json.load(trans_save)
mixed_save = open('./mytest/static/char_mixed.json')
char_mixed = json.load(mixed_save)
text_save = open('./mytest/static/char_text.json')
...
def fenci(request):
    ...
    print(text)
    change_sents = viterbi(text)
    mess = char_viterbi(change_sents)
    data = {
        'result' : mess,
    }
